...
void show_advanced_menu() {
    ...
    for (;;) {
        ...
        switch (chosen_item) {
			...
			case 8: {
                if (confirm_selection("Clear Misc Flag?", "Yes - Clear Misc flag")) {
                    if (1 == __system("/sbin/clearflag.sh"))
                        ui_print("Done!\n");
                    else
                        ui_print("Failed to clear misc!\n");
                }
                break;
            }
			case 9: {
                if (confirm_selection("Backup Misc?", "Yes - Backup Misc")) {
                    if (1 == __system("/sbin/backup.sh"))
                        ui_print("Done!\n");
                    else
                        ui_print("Failed to backup misc!\n");
                }
                break;
            }
			case 10: {
                if (confirm_selection("Restore Misc?", "Yes - Restore Misc")) {
                    if (1 == __system("/sbin/restore.sh"))
                        ui_print("Done!\n");
                    else
                        ui_print("Failed to restore misc!\n");
                }
                break;
            }
        }
